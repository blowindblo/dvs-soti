...
inspir <- df %>% 
  ...
  filter(inspiration != "")
inspir$inspiration <- trimws(inspir$inspiration) # remove trailing spaces
inspir$doc_id <- 1:nrow(inspir)
...
View(sep %>% filter(grepl('@',inspiration)))
...
# manually populating data (mostly just magazines/blogs/websites etc)
...
View(inspir %>% filter(grepl('makeover',inspiration)))

manual_extract <- 
  do.call(Map, c(f = replace_words, arguments)) %>% 
  do.call(rbind.data.frame, .) %>% 
  rbind(inspir %>%
          filter(grepl('beautiful', inspiration),
                 !grepl('information is beautiful',inspiration)) %>% 
          mutate(inspiration = 'r/dataisbeautiful', type = 'reddit')) %>% 
...
View(inspir %>% filter(grepl('tufte', inspiration)))
...
names <- names_complete %>% 
  mutate(token = case_when(token == 'few' ~ 'stephen few',
                           ...
                           grepl('rosling', token) ~ 'hans rosling',
...
View(final_data %>% group_by(inspiration) %>% summarise(count = n()))
